...
<script lang="ts">
  ...
  import {
    contAttachmentSaveApi,
    getContAttachmentListApi,
    contAttachmentUpdateFieldApi,
    contAttachmentDelApi,
  } from '/@/api/devsys/contract/collcontract';
  import { getdataListApi } from '/@/api/devsys/system/datadic';
  import { downloadByUrl } from '/@/utils/file/download';
  const uploadparam = { FolderIndex: 7 };
  ...
  export default defineComponent({
    ...
    setup(props: any) {
      ...
      function handleDownload(record: Recordable) {
        const loadurl = devconfig.devupload.baseurl + record.FILE_PATH;
        downloadByUrl({
          url: loadurl,
          target: '_self',
        });
      ...
      async function handleDelete(record: Recordable) {
        await contAttachmentDelApi({ Ids: record.ID.toString() });
      ...
      async function handleEditEnd(editdata) {
        ...
        await contAttachmentUpdateFieldApi(upobj);
